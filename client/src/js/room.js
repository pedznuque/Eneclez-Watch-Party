...
const autoCaptionsButton = document.getElementById("autoCaptions");
const autoCaptionOverlayEl = document.getElementById("autoCaptionOverlay");
...
let autoCaptionsLoading = false;
let autoCaptionSourceUrl = "";
let autoCaptionSegments = [];
let autoCaptionText = "";
let autoCaptionRequestId = 0;
let autoCaptionPollTimer = null;
let autoCaptionStopToken = 0;
let lastAutoCaptionLine = "";
let autoCaptionMode = "";
let appCaptionRecorder = null;
let appCaptionStream = null;
let appCaptionChunks = [];
let appCaptionMimeType = "audio/webm";
...
function getAutoCaptionSourceUrl() {
    const currentUrl = String(selectedVideoUrl || playerWebview.src || "").trim();

    if (isGoogleDriveVideoUrl(currentUrl)) {
        return getGoogleDriveApiStreamUrl(currentUrl);
    }

    if (isGoogleDriveApiStreamUrl(currentUrl)) {
        return currentUrl;
    }
...
function setAutoCaptionOverlay(text = "", options = {}) {
    if (!autoCaptionOverlayEl) return;

    const nextText = String(text || "").trim();
    autoCaptionOverlayEl.textContent = nextText;
    autoCaptionOverlayEl.hidden = !nextText;
    autoCaptionOverlayEl.classList.toggle("is-note", Boolean(options.note));
}

function setAutoCaptionsButtonState() {
    if (!autoCaptionsButton) return;

    autoCaptionsButton.classList.toggle("is-active", autoCaptionsEnabled);
    autoCaptionsButton.classList.toggle("is-loading", autoCaptionsLoading);
    autoCaptionsButton.disabled = autoCaptionsLoading;
    autoCaptionsButton.setAttribute("aria-pressed", String(autoCaptionsEnabled));
    autoCaptionsButton.title = autoCaptionsLoading
        ? "Generating captions"
        : autoCaptionsEnabled
            ? "Hide auto captions"
            : "Auto captions";
...
function resetAutoCaptions(options = {}) {
    autoCaptionRequestId += 1;
    autoCaptionStopToken += 1;
    clearTimeout(autoCaptionPollTimer);
    autoCaptionPollTimer = null;
    autoCaptionsLoading = false;
    autoCaptionSourceUrl = "";
    autoCaptionSegments = [];
    autoCaptionText = "";
    lastAutoCaptionLine = "";
    autoCaptionMode = "";
    ...
    setAutoCaptionOverlay("");
    setAutoCaptionsButtonState();
}

function updateAutoCaptionForTime(currentTime = 0) {
    if (!autoCaptionsEnabled || autoCaptionsLoading || !autoCaptionSegments.length) return;

    const time = Number(currentTime) || 0;
    const active = autoCaptionSegments.find(segment =>
        time >= Number(segment.start || 0) - 0.18 &&
        time <= Number(segment.end || 0) + 0.35
    );

    setAutoCaptionOverlay(active?.text || "");
}

function getCaptionWebview() {
    if (driveBrowserPromotedToPlayer && browserWebview && !browserWebview.hidden) {
...
async function installCaptionRecorder() {
    const targetWebview = getCaptionWebview();
    if (!targetWebview || targetWebview.src === "about:blank") {
        return {
            ok: false,
            error: "Load and play a video first."
        };
    }

    try {
        return await targetWebview.executeJavaScript(`
            (async () => {
                const pickVideo = () => {
                    const videos = Array.from(document.querySelectorAll("video"));
                    return videos
                        .filter(video => video.readyState >= 2 || video.videoWidth > 0 || Number.isFinite(video.duration))
                ...
                if (!video) {
                    return { ok: false, error: "No playable video found for captions." };
                }

                const capture = video.captureStream || video.mozCaptureStream;
                if (!capture) {
                    return { ok: false, error: "This video cannot expose audio for captions." };
                }

                if (window.__watchPartyCaptionRecorder?.recorder?.state === "recording") {
                ...
                if (!audioTracks.length) {
                    return { ok: false, error: "No audio track found yet. Start playback, then try CC again." };
                }
                ...
                window.__watchPartyReadCaptionChunk = async () => {
                    const session = window.__watchPartyCaptionRecorder;
                ...
                window.__watchPartyStopCaptionRecorder = () => {
                    const session = window.__watchPartyCaptionRecorder;
                    if (session?.recorder && session.recorder.state !== "inactive") {
                        session.recorder.stop();
                    }
                    session?.stream?.getTracks?.().forEach(track => track.stop());
                    window.__watchPartyCaptionRecorder = null;
                    window.__watchPartyReadCaptionChunk = null;
                    return true;
                };
        ...
        return {
            ok: false,
            error: error.message || "Could not start auto captions."
        };
    }
}

async function stopCaptionRecorder() {
    clearTimeout(autoCaptionPollTimer);
    autoCaptionPollTimer = null;
    autoCaptionStopToken += 1;

    if (appCaptionRecorder && appCaptionRecorder.state !== "inactive") {
        try {
            appCaptionRecorder.stop();
        } catch {}
    }
    appCaptionRecorder = null;
    appCaptionChunks = [];
    appCaptionStream?.getTracks?.().forEach(track => track.stop());
    appCaptionStream = null;
    autoCaptionMode = "";

    const targetWebview = getCaptionWebview();
    if (!targetWebview || targetWebview.src === "about:blank") return;

    await targetWebview.executeJavaScript(`
        (() => {
            try {
                return window.__watchPartyStopCaptionRecorder?.() || false;
            } catch {
                return false;
            }
        })()
...
async function installAppAudioCaptionRecorder() {
    if (!navigator.mediaDevices?.getDisplayMedia) {
        return {
            ok: false,
            error: "Screen audio capture is not available in this app."
        };
    }
    ...
    try {
        ...
        if (!audioTracks.length) {
            ...
            return {
                ok: false,
                error: "No audio was shared. Choose the app/screen and enable audio sharing."
            };
        }

        appCaptionMimeType = MediaRecorder.isTypeSupported("audio/webm;codecs=opus")
            ? "audio/webm;codecs=opus"
            : MediaRecorder.isTypeSupported("audio/webm")
                ? "audio/webm"
                : "";
        appCaptionStream = new MediaStream(audioTracks);
        appCaptionRecorder = new MediaRecorder(appCaptionStream, appCaptionMimeType ? { mimeType: appCaptionMimeType } : undefined);
        appCaptionMimeType = appCaptionRecorder.mimeType || appCaptionMimeType || "audio/webm";
        appCaptionChunks = [];

        appCaptionRecorder.ondataavailable = event => {
            if (event.data?.size > 0) {
                appCaptionChunks.push(event.data);
            }
        };
        appCaptionRecorder.onstop = () => {
            stream.getTracks().forEach(track => track.stop());
            appCaptionStream?.getTracks?.().forEach(track => track.stop());
        };
        ...
        return {
            ...
            error: error.name === "NotAllowedError"
                ...
                : error.message || "Could not start screen audio captions."
        };
    }
}

async function readAppAudioCaptionChunk() {
    if (!appCaptionRecorder || appCaptionRecorder.state === "inactive") {
        return {
            ok: false,
            error: "Caption recorder stopped."
        };
    }
    ...
    const blob = new Blob(appCaptionChunks.splice(0), { type: appCaptionMimeType });
    const buffer = await blob.arrayBuffer();
    const bytes = new Uint8Array(buffer);
    let binary = "";
    const batchSize = 0x8000;
    ...
    return {
        ok: true,
        audioBase64: btoa(binary),
        mimeType: blob.type || appCaptionMimeType
    };
}

async function readCaptionChunk() {
    if (autoCaptionMode === "app-audio") {
        return readAppAudioCaptionChunk();
    }
    ...
    try {
        return await targetWebview.executeJavaScript(`
            (async () => {
                if (!window.__watchPartyReadCaptionChunk) {
                    return { ok: false, error: "Caption recorder is not ready." };
                }
                return window.__watchPartyReadCaptionChunk();
            })()
        `);
    } catch (error) {
        return {
            ok: false,
            error: error.message || "Could not read caption audio."
        };
    }
}

async function transcribeCaptionChunk(chunk) {
    const response = await fetch(`${APP_SERVER_ORIGIN}/api/captions/transcribe-chunk`, {
        method: "POST",
        headers: {
...
async function checkCaptionServerReady() {
    try {
        const response = await fetch(`${APP_SERVER_ORIGIN}/api/captions/status`, {
            cache: "no-store"
        });
        const responseText = await response.text().catch(() => "");
        let payload = {};
        ...
        if (response.status === 404) {
            return {
                ok: false,
                error: "Caption server is not deployed yet. Deploy the latest server update."
            };
        }
        ...
        if (!response.ok) {
            return {
                ok: false,
                error: payload.error || responseText.slice(0, 160) || `Caption server failed (${response.status}).`
            };
        }
        ...
        if (!payload.configured) {
            return {
                ok: false,
                error: "Auto captions need GROQ_API_KEY in Render environment variables."
            };
        }
        ...
        return {
            ok: false,
            error: error.message || "Caption server is not reachable."
        };
    }
}
...
async function processCaptionChunk(token) {
    if (!autoCaptionsEnabled || token !== autoCaptionStopToken) return;

    const chunk = await readCaptionChunk();
    if (!autoCaptionsEnabled || token !== autoCaptionStopToken) return;
    ...
    if (chunk.empty || !chunk.audioBase64) {
        autoCaptionsLoading = false;
        setAutoCaptionsButtonState();
        scheduleCaptionChunk(token);
        return;
    ...
    try {
        const text = await transcribeCaptionChunk(chunk);
        if (!autoCaptionsEnabled || token !== autoCaptionStopToken) return;

        autoCaptionsLoading = false;
        setAutoCaptionsButtonState();

        if (text && text !== lastAutoCaptionLine) {
            lastAutoCaptionLine = text;
            setAutoCaptionOverlay(text);
            setTimeout(() => {
                if (autoCaptionsEnabled && lastAutoCaptionLine === text) {
                    setAutoCaptionOverlay("");
                }
            }, 5200);
        }
...
async function startLiveAutoCaptions() {
    autoCaptionsEnabled = true;
    autoCaptionsLoading = true;
    lastAutoCaptionLine = "";
    autoCaptionSegments = [];
    autoCaptionText = "";
    setAutoCaptionsButtonState();
    setAutoCaptionOverlay("Listening for speech...", { note: true });

    const serverReady = await checkCaptionServerReady();
    if (!serverReady.ok) {
        autoCaptionsLoading = false;
        setAutoCaptionsButtonState();
        setAutoCaptionOverlay(serverReady.error, { note: true });
        return;
    }
    ...
    if (!setup?.ok) {
        ...
        if (!setup?.ok) {
            autoCaptionsLoading = false;
            setAutoCaptionsButtonState();
            setAutoCaptionOverlay(setup?.error || "Auto captions could not start.", { note: true });
            return;
        }
    }
    ...
    setAutoCaptionsButtonState();
    setAutoCaptionOverlay(
        autoCaptionMode === "app-audio" ? "Auto captions listening to system audio" : "Auto captions on",
        { note: true }
    );
    setTimeout(() => {
        if (autoCaptionsEnabled) setAutoCaptionOverlay("");
    }, 1200);

    const token = autoCaptionStopToken;
    scheduleCaptionChunk(token);
}

async function loadAutoCaptions() {
    const sourceUrl = getAutoCaptionSourceUrl();
    const requestId = autoCaptionRequestId + 1;
    autoCaptionRequestId = requestId;

    if (!sourceUrl) {
        autoCaptionsEnabled = true;
        setAutoCaptionOverlay("Auto captions need a Drive video loaded in the player.", { note: true });
        setAutoCaptionsButtonState();
        return;
    }

    if (autoCaptionSourceUrl === sourceUrl && autoCaptionSegments.length) {
        autoCaptionsEnabled = true;
        setAutoCaptionsButtonState();
        updateAutoCaptionForTime(getDisplayPlaybackState(lastPlaybackState)?.currentTime || 0);
        return;
    }

    autoCaptionsEnabled = true;
    autoCaptionsLoading = true;
    autoCaptionSourceUrl = sourceUrl;
    autoCaptionSegments = [];
    autoCaptionText = "";
    setAutoCaptionsButtonState();
    setAutoCaptionOverlay("Generating auto captions...", { note: true });

    try {
        const response = await fetch(`${APP_SERVER_ORIGIN}/api/captions/transcribe`, {
            method: "POST",
            headers: {
                "content-type": "application/json"
            },
        ...
        if (requestId !== autoCaptionRequestId) return;

        if (!response.ok) {
            throw new Error(payload.error || "Auto captions failed.");
        }

        autoCaptionSegments = Array.isArray(payload.segments) ? payload.segments : [];
        autoCaptionText = String(payload.text || "");
        autoCaptionsLoading = false;
        setAutoCaptionsButtonState();

        if (!autoCaptionSegments.length && !autoCaptionText) {
            setAutoCaptionOverlay("No speech detected for captions.", { note: true });
            return;
        }
...
function updatePlaybackTime(state) {
    ...
    updateAutoCaptionForTime(currentTime);
}
...
autoCaptionsButton?.addEventListener("click", async () => {
    if (autoCaptionsLoading) return;
